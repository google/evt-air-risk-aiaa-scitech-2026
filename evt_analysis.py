...
class ExtremeValuesAnalysis:
  ...
  def __init__(
    ...
    self.fig_xi_vs_threshold = px.scatter(
        ...
        error_y="xi_se",
        labels={
            "threshold_cpa_neg": "Effective Threshold (Negative CPA Scale) [m]",
            "xi_hat": "Estimated ξ",
        },
  ...
  def _cdf(
      self, y: float, shape_parameter: float, selected_threshold: float
  ) -> float:
    """Computes the CDF of the GPD distribution."""
    return 1.0 - (1.0 + y / -selected_threshold) ** (-1 / shape_parameter)

  def empirical_cdf_plot(
      self, sample_cpas: list[float], selected_threshold: float
  ) -> go.Figure:
    """Generates a plot comparing fitted PDF to empirical CDF.

    Given a sample of CPAs assumed to fit the model generated by this analysis,
    this method plots the empirical CDF of the sample CPAs against the fitted
    PDF. This comparison aids in validating the gooness-of-fit of the model.

    Args:
        sample_cpas: A list of CPA values to be used for validation.
        selected_threshold: The threshold value to be used for validation.

    Returns:
        A Plotly graph object figure containing the CDF plots.

    Raises:
        ValueError: If no samples exceed the threshold.
    """
    selected_result = self.results_at_threshold(selected_threshold)

    v_cdf = np.vectorize(self._cdf)
    x_fit = np.linspace(0.0, selected_threshold)
    y_fit = v_cdf(x_fit, selected_result["xi_hat"], selected_threshold)

    sample_exceedances = [
        -x + selected_threshold for x in sample_cpas if x <= selected_threshold
    ]

    if not sample_exceedances:
      raise ValueError("No samples over the threshold.")

    # Plot the CDF and ECDF of the sample CPAs
    fig_cdf_ecdf = go.Figure()

    fig_cdf_ecdf.add_trace(
        go.Scatter(
            x=x_fit,
            y=y_fit,
            name="Fitted GPD function CDF",
        )
    )
    fig_cdf_ecdf.update_xaxes(range=[0.0, selected_threshold])
    # To compute the empirical CDF, sort the sample values in ascending order;
    # the cdf(sample_x) = ordinal(sample_x) / len(sample_x), 1-indexed.
    fig_cdf_ecdf.add_trace(
        go.Scatter(
            x=sorted(sample_exceedances),
            y=np.linspace(0.0, 1.0, len(sample_exceedances) + 1)[1:],
            mode="lines+markers",
            name="Empirical CDF",
        )
    )

    fig_cdf_ecdf.update_layout(
        title=dict(text="CDF and ECDF Comparison"),
        xaxis=dict(title=dict(text="Exceedence over threshold [m]")),
        yaxis=dict(title=dict(text="CDF")),
    )

    return fig_cdf_ecdf

  def ks_test_model_fit(
      self, sample_cpas: list[float], selected_threshold: float
  ) -> tuple[Any, go.Figure]:
    """Compares the estimated PDF to a set of CPAs gathered from real-world data.

    This method validates the results of the analysis by comparing the
    estimated PDF to a set of sample CPAs (ideally from real-world data). It
    determines whether the estimated PDF is sufficiently close to the sample
    CPAs to be considered valid.
    ...
    Returns:
        result: A KstestResult object containing the results of
        the KS test. Note the type check is Any because the KstestResult
        class is not visible to this module.
    ...
    return stats.kstest(
        sample_exceedances,
        self._cdf,
        args=(selected_result["xi_hat"], selected_threshold),
    )
